@echo off
REM Musikku offline installer for Windows.
...
echo.
echo === Musikku offline installer ===
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ForceUpdateFromAnyVersion -ErrorAction Stop; exit 0 } catch { Write-Host 'In-place update failed; removing existing Musikku and reinstalling...'; Get-AppxPackage -Name 'Simpmusic' | Remove-AppxPackage -ErrorAction SilentlyContinue; try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ErrorAction Stop; exit 0 } catch { Write-Host $_; exit 1 } }"
...
REM --- 4. Launch app ---------------------------------------------------------
echo.
echo [3/3] Launching Musikku ...
start "" "shell:appsFolder\Simpmusic_ejp2bhxmz1qq6!Simpmusic"

echo.
echo Musikku installed successfully.
echo To uninstall: Settings -^> Apps -^> Installed apps -^> Musikku -^> Uninstall
echo.
pause
endlocal
