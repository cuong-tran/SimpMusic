...
- **Background playback resumption (2026-08-26)**: `SimpleMediaSessionCallback.onPlaybackResumption` was never implemented, so any `play` that arrived with an empty player - a Samsung Routine, a Bluetooth or headset play button, the system resumption chip - killed the service. `MediaButtonReceiver` starts `SimpleMediaService` with `startForegroundService()`; Media3's default `onPlaybackResumption` returns `Futures.immediateFailedFuture(UnsupportedOperationException())`, logs it, and still runs `prepare() + play()`, but an empty player lands in ENDED rather than BUFFERING, `MediaNotificationManager.isAnySessionUserEngaged()` stays false, and `startForeground()` is never called. **Nothing is blocked** - the ANR trace is just `nativePollOnce` on an idle main thread - the service simply misses the foreground-start deadline and is torn down with "Context.startForegroundService() did not then call Service.startForeground()", which Samsung/Sentry report as a background ANR.
  - The queue is restored through `MediaPlayerHandler.restoreQueueAndPlay()` (new), not handed back to Media3. The session player is a single-item delegate in front of `CrossfadeExoPlayerAdapter`, which owns the real playlist, so a playlist pushed in through `setMediaItems` would desync the two - the callback returns an empty list with `C.INDEX_UNSET`, the same contract `onSetMediaItems` already uses for Android Auto. That empty return is also why the restore is started on a *later* main-thread message: Media3 runs its success handling (`setMediaItems(emptyList())` + `prepare()` + `play()`) inline the moment the future completes and would clear a player primed too early.
  - `mayBeRestoreQueue()` and the resumption path share one mutex-guarded `restoreSavedQueue(playWhenReady)`, because the service start that dispatches the media button also creates the handler, whose init restores the queue - that race is the normal ordering, not an edge case. The play request is handed to the in-flight restore through a flag rather than waiting for the lock, so the first note does not queue behind `load()`, which paces itself in 100-track chunks. Resumption therefore plays a moment of the track before the tail seek lands the saved position.
  - `keepServiceAlive`'s `runBlocking` DataStore read moved off `onCreate`: a blocking disk read on the main thread spends part of the same deadline.
