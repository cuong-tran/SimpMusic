...
- **Musikku app icon (2026-09-15)**: the whole icon set is regenerated from the design artwork (a navy seigaiha field carrying a red maple leaf with a cream eighth note, plus a gold ring and blossoms) — launcher, legacy, desktop, Play Store, AppImage, and the two in-app logo vectors. The wordmark in the source is deliberately **not** in any icon: it is illegible below ~96 px, so every asset carries the emblem alone.
  - **The emblem is placed by its MINIMAL ENCLOSING CIRCLE, never its bounding box.** It is a circular badge whose leaf stem pokes below the ring, so the bbox centre sits ~7 px high and the mark visibly drifts up the moment any circular mask is applied. `extract.py` measures the circle into `work/emblem_geometry.json` and `gen.py` reads it — restating it in `gen.py` is how the two drift apart on the next re-export, and the failure is silent: every icon simply places the emblem slightly wrong.
  - **The design layers are `composeApp/icon/logo_transparent.png` + `background.png`**; `logo_source.png` is the full lockup, kept as reference and as the wordmark's source. Two traps seen in earlier exports of the emblem layer, both of which produce plausible output rather than an error, so `extract.py` still guards against both. **Check that the alpha is real**: `logo_only.png` had alpha 1.0 everywhere with the transparency checkerboard painted into the pixels (#454545 / #757575, ~25 px tiles) — identical in any viewer, and it silently bakes a grey checkerboard behind the emblem. **The fallback keying cannot be a colour fuzz**: the leaf's dark maroon outline (74,31,38) sits 19% from #454545 in RGB distance, so the fuzz that finally crosses the checkerboard (18%) also eats the outline and punches holes through the leaf; chroma separates them cleanly, the checkerboard being exactly neutral and the one low-chroma emblem colour (the cream note) far too bright to collide. And **stray background decoration must be dropped before anything is measured** — an earlier layer carried a lone sparkle ~400 px from the emblem, and an enclosing circle that includes it is nearly twice the right radius, so every icon would place the emblem at half its intended size. Components are kept by proximity, not size: ring arcs and blossoms are each their own component. When the alpha IS real it is used as-is, with the artist's anti-aliasing untouched — eroding it would visibly thin the ring and the blossoms.
  - **The adaptive background moved from `@drawable` to `@mipmap`.** It was still Android Studio's green `#3DDC84` template vector — the icon shipped a white foreground plate that happened to cover it completely. It is now a raster in all five densities so the launcher carries the same wave texture as the desktop and store icons, both `mipmap-anydpi-v26` XMLs point at `@mipmap/ic_launcher_background`, and `drawable/ic_launcher_background.xml` is deleted. The copy in `composeResources` is unused by anything and was flattened to solid navy rather than removed.
  - **Encoding is lossy on purpose.** Lossless WebP put the five densities × four layers at 750 kB; `cwebp -q 90/92 -alpha_q 100` lands at 260 kB with PSNR 40.7/45.7 dB, which is invisible at icon scale. The three in-app PNGs are `PNG8` at 255 colours with Floyd–Steinberg (83/83/77 kB) so the APK footprint matches the ~70 kB each they replaced. `composeApp/icon/circle_app_icon.png` stays 24-bit at 1024 — it is the Conveyor source and never enters the APK.
  - **`mono.xml` / `monochrome.xml` are traced, not hand-drawn.** No tracer is installed on this machine (no potrace/autotrace), so `scripts/icons/trace.py` does it (`scripts/icons/README.md` has the whole pipeline): leaf selected by colour → opened → cream sub-blob for the note → light raster blur + re-threshold (σ 2.6 / 2.0) → crack-following contour trace → resample → contour smoothing → RDP ε 1.6 → Catmull-Rom.
  - **The edge wobble is removed in CONTOUR space; raster blur cannot do it.** Blur only takes out the pixel staircase and is isotropic, so pushed far enough to erase the source art's sketchy edge it also melts the lobe tips — the first version shipped at σ 3.2 / ε 1.9 / 48° corners and traced *visibly faceted*, straight segments meeting at angles all along what should be smooth curves. Each contour is now resampled at uniform arc length (5 px), its genuine corners found as local maxima of the turn measured across **±5 samples** (≥58°), and each run BETWEEN corners Gaussian-smoothed with **both ends pinned** (σ 6.5 samples). Pinning is the whole trick: it lets σ go high enough to erase the wobble while the lobe tips and the valleys between them stay sharp. Result is 85 curve segments instead of 106, and a smaller path.
  - **Measure the turn across a WINDOW, never between neighbouring points.** On a resampled contour a per-vertex angle is mostly quantisation noise, which is exactly why the old 48° per-vertex test fired on smooth edges and produced the facets.
  - **The leaf cannot be found as "the largest opaque blob" — the gold ring touches it**, so that blob is leaf + ring + blossoms and traces as a scribble. Red separates them with room to spare (every leaf tone from the bright face `226,74,60` to the dark outline `64,4,6` clears `r-g > 25`, while the ring is 21 and the cream note 4), but the ring's OWN dark stroke is red enough to pass — so a 3 px erode/dilate opening drops the ~3 px arc still hanging off the stem. On a 400 px leaf the opening is invisible.
  - Both vectors fit the mark to a **circle**, not the viewport box, because both consumers clip to one: `mono` at r 45/54 (the nav-rail logo is `Image` + `CircleShape`, and it doubles as the notification small icon, where the system reads only alpha), `monochrome` at r 33/54 (the adaptive 66 dp safe zone). A box fit slices the outer lobes off.
  - **`core/` is a git submodule**, so `core/media/media3/src/main/res/drawable/mono.xml` needs its own commit in `cuong-tran/simpmusic_core` before the superproject pointer can move. The three `mono.xml` copies and the two `monochrome.xml` copies must stay byte-identical.
  - `composeApp/appimage/simpmusic.png` is the icon that actually reaches Linux users: `packageConveyorAppImage` only copies `circle_app_icon.png` over it `if (!iconDst.exists())`, and it does exist. Not updating it leaves the AppImage on the old logo with no error anywhere.
  - **The two banners are `scripts/icons/banner.py`, and they take OPPOSITE grounds on purpose.** `featureGraphic.png` (1024×500, both locales) gets the navy field — it is only ever an image. `dmg-background.png` (2800×1600, the 2× of create-dmg's 1400×800 window) stays LIGHT, because Finder draws the icon labels itself in the system appearance's text colour and no background image can influence that: dark text on a navy ground is what a light-mode Mac renders, and that is most of them. The wordmark, being cream with a maroon outline, gets a navy plate on that light ground rather than being recoloured, so the brand's own lettering survives.
  - **The DMG layout is `scripts/wrap-mac-dmg.sh`'s geometry, not a free composition**: icon slots at (350,380) and (1080,360) at 192 px in the 1400×800 window, so artwork stays above y≈284 and below y≈500 or it collides with an icon or its label.
  - **The wordmark is keyed on BLUE-MINUS-RED, not colour fuzz.** A corner flood fill at 20% leaves a rectangular patch of the source's navy sitting behind the letters — the field is subtly graded and the fill runs out of tolerance before it reaches there — and on the finished banner that reads as a box around the wordmark. Navy (+35), cream (−13) and the maroon outline (−52) separate cleanly on b−r, and a ramp rather than a threshold keeps the glyph edges anti-aliased.
  - **Futura silently drops Vietnamese diacritics** — it renders "Không chỉ là miễn phí khi nghe nhạc" with three glyphs simply missing, no warning. `SFNSRounded` covers them and matches the wordmark's rounded shapes. Taglines are fitted to ONE line by stepping the point size down: letting the caption wrap orphaned "nhạc" onto a second line.
  - **A PNG8 write attaches a tRNS chunk if the alpha channel is merely opaque rather than absent**, and Play rejects a feature graphic with transparency. Both banners are written with `-alpha remove -alpha off` plus `png:exclude-chunk=tRNS`; verified by reading the chunk list, since `%[channels]` reports the in-memory representation and says `srgb 4.0` either way.
  - The DMG's `<name>.dragTo(Applications)` line names the real bundle, so it moved with the desktop rename below; `banner.py` defaults to `Musikku` and takes the name as its first argument.

- **Desktop app renamed to Musikku (2026-09-15)**: the Android launcher had been renamed in `fdf83c24` and nothing else had, so the desktop build still produced `SimpMusic.app` while the icon said Musikku. The rename follows the shape that commit set: **every user-visible name moves, every identifier stays.**
  - Moved: `app.display-name` (names the macOS `.app` bundle, the Windows Start-menu entry and the Linux `.desktop` `Name`), `packageName` in compose `nativeDistributions`, the AppImage filename (`appName` in `desktopApp/build.gradle.kts`), `start-menu.group`, the create-dmg `--volname` and its `/Volumes/…` stale-mount sweep, `composeResources/values/app_name.xml`, and the `.dmg` / Windows-installer-zip names in both workflows.
  - **Kept, deliberately: `app.fsname` (`simpmusic`) and `app.rdns-name` (`com.maxrave.simpmusic`)** — the same call as keeping `applicationId` on Android. They are what every artifact filename and package identity hangs off: `bin/simpmusic`, `simpmusic.crt`, `simpmusic-*.x64.msix`, the mac zips (`output/*.zip`, named from fsname — which is why `mac-zips/simpmusic-*-mac-*.zip` still matches), the MSIX identity `Simpmusic_ejp2bhxmz1qq6!Simpmusic` that `install.bat` launches, and the AppImage's `Icon=simpmusic` / `Exec=bin/simpmusic`. Renaming fsname means renaming all of those in lockstep, and a missed glob fails a release job with "no such file" — or worse, finds nothing and carries on.
  - **`x-scheme-handler/simpmusic` and `url-schemes` must NOT be renamed**: they are the deep-link scheme (`simpmusic://`, backing simpmusic.org links), not a name. Neither may `com-maxrave-simpmusic-MainKt.desktop`, the filename AppRun installs into `~/.local/share/applications` — keeping it means an upgrade overwrites the old entry instead of leaving two.
  - **StartupWMClass exists in THREE places and all three must agree**, or GNOME/KDE stops binding the launcher icon to the window: `forceLinuxWmClass()` in `desktopApp/.../Main.kt` (which sets it by reflection because the JDK has no property for it), `conveyor.conf`'s `desktop-file."Desktop Entry"`, and the `.desktop` that `packageConveyorAppImage` writes — the last being the one that actually reaches AppImage users.
  - `composeResources` `app_name.xml` is shared, so this also renames the Android home-screen header, the Credits screen, the settings copy and the desktop window/tray titles. Android's launcher label lives separately in `androidApp/src/main/res/values/app_name.xml` and was already Musikku.
  - The Play listing moved with it: `title.txt` and the opening tagline of `full_description.txt` in both locales. **`SimpMusic Lyrics` in the feature list stays** — it is a third-party provider (`api-lyrics.simpmusic.org`, upstream's service, listed there beside LRCLIB), not the app. `short_description.txt` never named the app. The **changelogs are history and are not rewritten**: each one records what shipped under the old name. The English tagline was a pun ("Not a simple music player, it's SimpMusic") so it was reworded rather than word-swapped — leaving "simple" in would echo a name that is no longer there; Vietnamese never leaned on it and is a straight swap. These files carry **no trailing newline**, and fastlane sends the bytes verbatim.
  - Still on the old name and intentionally so: `install.bat`'s payload globs and package-identity strings.
